                ...
                <div className="mt-12">
                    ...
                    <section>
                        <div className="flex justify-between items-baseline mb-4">
                            <h3 className="text-2xl font-semibold font-serif">Albums</h3>
                            {albums.length > TOP_ALBUMS_COUNT && (
                                <NavLink to={`/artist/${artist.id}/albums`} className="text-sm font-semibold text-ac-secondary hover:underline">See all</NavLink>
                            )}
                        </div>
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-6 gap-y-10">
                            {albums.slice(0, TOP_ALBUMS_COUNT).map(album => <AlbumCard key={album.id} album={album} />)}
                        </div>
                    ...
                    <section className="mt-12">
                         <div className="flex justify-between items-baseline mb-4">
                            <h3 className="text-2xl font-semibold font-serif">Songs</h3>
                            {songs.length > TOP_SONGS_COUNT && (
                                <NavLink to={`/artist/${artist.id}/songs`} className="text-sm font-semibold text-ac-secondary hover:underline">See all</NavLink>
                            )}
                        </div>
                        <div className="rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
                            <ul className="divide-y divide-gray-200 dark:divide-gray-700">
                                {songs.slice(0, TOP_SONGS_COUNT).map(song => (
                                    <li key={song.id} className="transition-colors odd:bg-transparent even:bg-black/[.03] dark:even:bg-white/[.03] hover:bg-black/[.05] dark:hover:bg-white/[.05]">
                                        <NavLink to={`/song/${song.id}`} className="flex items-center space-x-4 p-3 w-full">
                                            <img 
