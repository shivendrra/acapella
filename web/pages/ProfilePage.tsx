            ...
            <div className="flex items-center justify-between">
              ...
              {currentUserProfile && currentUserProfile.uid !== user.uid && (
                <button
                  ...
                  className={`px-3 py-1 text-xs font-semibold rounded-full ${followingIds.has(user.uid)
    ...
    const fetchProfileData = async () => {
      ...
      try {
        ...
        if (currentUserProfile) {
          ...
          // Filter likes to only show Songs and Albums (exclude reviews)
          const likesQuery = query(
            collection(db, 'likes'),
            where('userId', '==', userProfileData.uid),
            where('entityType', 'in', ['song', 'album']), // Only fetch songs/albums
