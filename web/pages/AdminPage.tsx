...
// FIX: Added getDoc to firebase imports to resolve "Cannot find name 'getDoc'" error.
...
import { X, Plus, Edit, Trash2, Link as LinkIcon, Search } from 'lucide-react';
import { formatDate } from '../utils/formatters';

// FIX: Added debounce utility function to resolve "Cannot find name 'debounce'" error.
...
const ManageContent: React.FC<{
    ...
    const fetchData = useCallback(async () => {
        ...
        const snapshot = await getDocs(q);
        // FIX: Cast doc.data() to object to resolve "Spread types may only be created from object types" error. The original error line number was likely incorrect.
        setItems(snapshot.docs.map(doc => ({ id: doc.id, ...(doc.data() as object) })));
        setLoading(false);
    }, [collectionName, searchTerm]);

    useEffect(() => {
        // Initial fetch or when search term is cleared
        if (!searchTerm) {
            fetchData();
        }
                    ...
                    <div>
                     ...
                     <div>
                        ...
                        <MultiSelectCombobox
                           ...
                           placeholder="Search & select songs..."
                        />
                    </div>
                    <div>
                        <label className="block text-sm font-medium mb-1">Platform Links</label>
    ...
    useEffect(() => {
        const fetchInitialAlbum = async () => {
            if (item?.albumId) {
                ...
                if (albumSnap.exists()) {
                    setAlbumOptions([{ id: albumSnap.id, name: albumSnap.data().title }]);
                }
            }
        };
        fetchInitialAlbum();
...
const AdminPage: React.FC = () => {
    ...
    const renderContent = () => {
        switch (activeTab) {
            ...
            case 'albums':
                return <ManageContent collectionName="albums" title="Manage Albums" FormComponent={AlbumFormModal} />;
            case 'songs':
                 return <ManageContent collectionName="songs" title="Manage Songs" FormComponent={SongFormModal} />;
            default:
                return null;
        }
    };
